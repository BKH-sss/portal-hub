...
> **최종 릴리즈 일시 (Last Updated)**: `2026-09-09 09:30:00 KST`  
> **최신 버전 (Current Version)**: `v3.8.0`

---

## 🚀 [2026-09-09 09:30 KST] v3.8.0 - 구글 캘린더 실시간 15분 주기 자동 동기화, 모닝 케어 브리핑 연동 및 전 모듈 철통 SafeRouter 방어 체계 구축

### 📌 패치 개요
- **증상 해결**:
  1. 구글 캘린더에 일정이 정상 등록되어 있음에도 매일 아침 "스카디의 모닝 케어" 브리핑에 항상 *"오늘 예정된 굵직한 일정은 없어"*라고만 출력되던 버그 완벽 해결.
  2. 모닝 브리핑 생성 직전 및 백그라운드 15분 주기로 구글 캘린더 iCal 피드를 자동 동기화하여 SQLite 데이터베이스에 최신 상태 유지.
  3. 구글 캘린더 iCal 피드의 UTC(Z) 시각이 KST(+9시간)로 정확히 변환되도록 날짜/시간 파서 전면 개편 (새벽/오전 일정이 전날로 누락되던 시차 롤오버 버그 원천 해결).
  4. RFC 5545 표준에 따른 Line Unfolding, `STATUS:CANCELLED` 취소 일정 자동 필터링, UID 기반 멱등성 보장 Upsert(중복 등록 방지) 탑재.
  5. 배포 환경(Render 등)에서 FastAPI/Pydantic 미설치 시 모듈 로드 단계에서 프로세스가 비정상 종료(Exit 1)되던 문제를 차단하기 위해, 전 20개 모듈에 대해 무중단 더미 라우터를 제공하는 `_safe_router.py` 가드레일 체계 구축.
- **적용 일시**: **2026년 09월 09일 (수) 09:30 KST**
- **릴리즈 버전**: `v3.8.0`
...
1. **구글 캘린더 iCal 실시간 15분 주기 자동 동기화 루프 (`@tasks.loop(minutes=15)`)**:
   - `discord_bot/discord_skadi_bot.py`: `google_calendar_sync_task` 탑재 및 `on_ready` 시 즉시 비동기 1회 자동 동기화 + 15분 주기 백그라운드 워치독 가동.
   - `discord_bot/skadi_personal_care.py`: 08:00 모닝 케어 및 23:00 나이트 케어 발송 직전 구글 캘린더 피드를 자동 동기화하여 최신 일정 브리핑 반영.
   - `generate_morning_briefing_content()`: 모닝 브리핑 전송 전 자동 동기화 수행.
2. **KST(+9시간) 정확한 타임존 변환 & RFC 5545 파서 고도화 (`schedule_manager.py`)**:
   - UTC `20260908T230000Z` -> KST `2026-09-09 08:00` 날짜 롤오버 정확 변환.
   - 종일 일정 (`VALUE=DATE:YYYYMMDD`) 및 로컬 시각 포맷 정밀 파싱.
   - 줄바꿈 언폴딩(`\r?\n[ \t]`) 및 `STATUS:CANCELLED` 일정 자동 배제.
   - `uid` 컬럼 자동 마이그레이션 및 인덱스 생성, UID 기반 중복 없는 업데이트.
3. **전 모듈 무중단 철통 가드레일 (`_safe_router.py`)**:
   - `modules/_safe_router.py`, `discord_bot/_safe_router.py`: FastAPI/Pydantic 부재 시에도 모든 데코레이터(`get`, `post`, `put`, `delete` 등)와 모델 클래스를 무결점 더미 객체로 대체하여 프로세스 크래시 원천 차단.
   - 20개 전체 확장 모듈 안전 로드 적용 완료.
4. **실시간 동기화 진단 보고 기능 탑재 (`!상태`, `!버전`)**:
   - 최근 동기화 시각, 동기화 성공 여부, 피드 이벤트 수, 로컬 DB 등록 일정 수를 한눈에 확인 가능.

---

## 🚀 [2026-09-08 12:48 KST] v3.7.0 - 구글 캘린더(Google Calendar) 기반 자연어 일정 조회/추가 & 시작 10분 전 자동 사전 알림 시스템 전면 구축

### 📌 패치 개요
- **개발 목적**: 유저가 `@SKADI 13시 일정 알려줘` 등 자연어로 일정을 물었을 때 캘린더를 조회하지 않고 환각성 감성 대사로 둘러대던 이슈를 완벽하게 해결하고, **구글 캘린더(Google Calendar) 기준으로 일정을 조회/추가**하며 **일정 시작 정확히 10분 전에 1:1 개인 DM으로 알잘딱깔센하게 사전 알림**을 발송하는 전용 스케줄 엔진 전면 가동.
- **적용 일시**: **2026년 09월 08일 (화) 12:48 KST**
- **릴리즈 버전**: `v3.7.0`
- **핵심 가치**:
  - **자연어 구글 캘린더 일정 조회 (`format_schedule_query_response`)**:
    - `"13시 일정 알려줘"`, `"오늘 일정 뭐야"`, `"내일 일정 확인"` 등 자연어 시간 질의 자동 파싱
    - 특정 시간대(예: 13시)에 예정된 일정이 있으면 정확한 일정명과 메모, 10분 전 사전 알림 예정 시각을 브리핑
    - 해당 시간에 일정이 없는 경우, 지어내지 않고 정직하고 다정하게 *"13시에는 등록된 일정이 없어. 자유 시간이야!"*라고 안내하며 다른 일정 및 추가 방법 안내
  - **자연어 구글 캘린더 일정 추가 (`add_schedule_from_text`)**:
    - `"오늘 13시에 회의 일정 추가해줘"`, `"내일 15시 치과 예약 등록해줘"` 등 일시와 제목을 똑똑하게 파싱
    - 스케줄 영구 DB 및 구글 캘린더 자동 등록, 스마트폰 구글/삼성 캘린더 앱에 원클릭으로 들어가는 템플릿 URL 즉시 발급
  - **구글 캘린더 일정 시작 10분 전 자동 사전 알림 (10-Minute Prior Alert)**:
    - 1분 주기 백그라운드 워치독(`personal_dm_care_task`)이 당일 구글 캘린더 일정을 실시간 감시
    - 일정 시작 10분 전(0초 < 남은 시간 <= 600초)이 도래하면 마스터 1:1 개인 DM으로 정갈하고 다정한 사전 알림 자동 발송
    - 중복 발송 방지 및 알림 이력 영구 관리 (`data/skadi_calendar_alerts.json`)
  - **LLM 시스템 프롬프트 구글 캘린더 실시간 주입 (Anti-Hallucination Grounding)**:
    - 자유 대화에서도 일정을 지어내지 않고 실제 등록된 캘린더 데이터 기반으로 답변하도록 프롬프트 레벨 팩트 바인딩 완료
  - **구글 캘린더 양방향 연동 명령어 탑재 (`!구글캘린더`, `!구글캘린더 연동 [iCal URL]`, `!구글캘린더 동기화`)**:
    - 구글 캘린더 비공개 iCal 주소 등록 시 자동 실시간 동기화 지원
...
| :--- | :---: | :--- |
| [`modules/google_calendar_engine.py`](file:///C:/Users/Su-Bla/orca/workspaces/NEO/char/modules/google_calendar_engine.py) | **신규 (독립)** | **구글 캘린더 통합 매니저 및 자연어 처리/10분 전 사전 알림 코어**<br>• 자연어 날짜/시간 파서, 구글 캘린더 템플릿 URL 생성기<br>• 10분 전 사전 알림 감지 워치독 및 영구 이력 캐시 관리 |
| [`discord_bot/discord_skadi_bot.py`](file:///C:/Users/Su-Bla/orca/workspaces/NEO/char/discord_bot/discord_skadi_bot.py) | **업데이트** | **구글 캘린더 자연어 질의/추가 라우터 및 10분 전 DM 알림 연동**<br>• `on_message` 내 자연어 일정 조회/추가 핸들러 탑재<br>• `personal_dm_care_task` 내 10분 전 사전 알림 백그라운드 발송기 탑재<br>• `build_system_prompt()` 내 당일 구글 캘린더 일정 실시간 주입 (환각 방지)<br>• `!구글캘린더`, `!일정`, `!일정추가` 명령어 고도화 |
