...
# Windows 환경에서 콘솔 유니코드 이모지 출력 시 cp949 인코딩 에러 방지
...
@app.get("/skadi_chess.html", summary="체스 게임 UI 서빙")
def read_chess():
    return FileResponse("skadi_chess.html")

@app.get("/global", summary="ORBIS 해외 외신 포털 서빙")
@app.get("/global/", summary="ORBIS 해외 외신 포털 서빙 (슬래시)")
def read_global_portal():
    for cand in [os.path.join("portal", "global", "index.html"), os.path.join("global", "index.html"), "global.html"]:
        if os.path.exists(cand):
            return FileResponse(cand)
...
def read_static_file(filename: str):
    """HTML, CSS, JS, 이미지 등 정적 웹 에셋을 안전하게 검증하여 서빙합니다 (디렉터리 탈출 방어)."""
    # 1. 루트 폴더 내 허용된 정적 파일 검증
    path = safe_static_path(".", filename)
    if path:
        return FileResponse(path)

    # 2. images 폴더 내 허용된 이미지 파일 검증
    path = safe_static_path("images", filename)
    if path:
        return FileResponse(path)

    # 3. 비인가 또는 존재하지 않는 경로는 안전하게 메인 포털로 폴백
    return FileResponse("index.html")
