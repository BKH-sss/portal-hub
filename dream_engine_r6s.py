...
def dream_and_reflect(profile_data):
    ...
    prompt = f"""
    너는 최고의 E-sports 전술 분석가이자 레인보우 식스 시즈 전문가야.
    ...
    [분석 규칙]
    1. 원인 분석: 마스터가 어떤 맵, 어떤 상황(진입/방어)에서 취약했는지 찾아라.
    2. 실전 지침: 내일 당장 써먹을 수 있는 거친 피드백 1~2줄을 생성하라.
...
def run_learning_cycle():
    ...
    if profile_data and len(profile_data.strip()) > 10:
        ...
        write_log("❄️ 작전 데이터가 부족해. 무리하게 움직이지 마. (학습 데이터 없음)")
...
def run_scheduler():
    ...
    while is_running:
        ...
        wait_seconds = 30
        write_log(f"❄️ 다음 브리핑 전까지 {wait_seconds}초 대기할게...")
...
if __name__ == "__main__":
    ...
    if args.run_once:
        write_log(f"⚡ [강제 실행 모드] 수면 학습 (r6s) 1회 즉시 실행")
        run_learning_cycle()
        write_log("✅ 강제 실행 완료.")
    else:
        run_scheduler()
