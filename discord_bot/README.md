...
| `!일정 [날짜]` | `!스케줄` | 오늘 또는 지정일(YYYY-MM-DD) 일정 목록 확인 |
| `!일정추가 <일시> <제목>` | `!add_schedule` | 새 일정 캘린더 등록 (예: `!일정추가 2026-09-03 15:00 회의`) |
| `!일정삭제 <ID>` | `!del_schedule` | 등록된 일정 삭제 |
| `!할일` | `!todos`, `!할일목록` | 진행 중인 할 일(Todo) 체크리스트 확인 |
| `!할일추가 <내용>` | `!add_todo` | 할 일 추가 (긴급 시 `[긴급]` 태그 포함) |
| `!할일완료 <ID>` | `!done`, `!체크` | 할 일 완료/미완료 토글 |
| `!할일삭제 <ID>` | `!del_todo` | 할 일 삭제 |
| `!브리핑` | `!모닝브리핑` | 날씨 + 테마 뉴스 + 오늘 스케줄 & 할 일 브리핑 즉각 호출 |
