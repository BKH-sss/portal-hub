...
def dream_and_reflect(profile_data):
    ...
    prompt = f"""
    너는 천재 코더 '스카디 코더'야.
    아래는 마스터(유저)가 오늘 남긴 코딩 에러 로그 및 개발 기록이야.

    [오늘의 개발 기록]
    {profile_data}

    [분석 규칙]
    1. 원인 분석: 마스터가 어떤 논리적 오류나 코드 구조적 결함에 자주 부딪혔는지 찾아라.
    2. 실전 지침: 내일 코딩할 때 바로 약점을 보완할 수 있는 한 줄 지침을 거칠고 직설적인 반말로 생성하라.

    [출력 형식]
    분석 결과: [에러 및 취약점 요약].
    실전 지침: [내일 코딩에 바로 써먹을 거친 피드백 1~2줄]
    """
...
def run_learning_cycle():
    ...
    if profile_data and len(profile_data.strip()) > 10:
        ...
        write_log("💻 에러 로그가 없네? 웬일로 코드를 안 망쳤어? (학습할 에러 없음)")
...
def run_scheduler():
    ...
    while is_running:
        ...
        wait_seconds = 30
        write_log(f"💻 네가 뻘짓하기 전까지 {wait_seconds}초만 눈 붙인다...")
...
if __name__ == "__main__":
    ...
    if args.run_once:
        write_log(f"⚡ [강제 실행 모드] 수면 학습 (coding) 1회 즉시 실행")
        run_learning_cycle()
        write_log("✅ 강제 실행 완료.")
    else:
        run_scheduler()
